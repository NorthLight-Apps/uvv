...
if($_SERVER['REQUEST_METHOD']=='POST'){
 if(isset($_POST['login'])){
  ...
  if(strtolower($sName)==ADF_Admin&&$sPass==fFraDeCode(ADF_Passwort)){//Zugangsdaten OK
...
if($_SERVER['REQUEST_METHOD']!='POST'){//GET
 if(!isset($_SESSION['Id'])||$_SESSION['Id']!=md5($sId)){
  echo '<p class="admMeld" style="text-align:center">Weisen Sie sich als berechtigter Administrator aus.</p>';
  echo fLoginForm();
 }else{
...
Somit wird die Anmeldung als Administrator wahrscheinlich nicht funktionieren.
