...
$sFraSelf=(isset($_SERVER['SCRIPT_NAME'])?$_SERVER['SCRIPT_NAME']:(isset($_SERVER['PHP_SELF'])?$_SERVER['PHP_SELF']:'f_Freischalt.php'));
...
if(substr($sFraSelf,-16)=='f_Freischalt.php'){ //direkter Aufruf
...
function fFraSeite(){ //Seiteninhalt
 ...
 $sForm='LoginForm'; $sBtn=FRA_TxAnmelden; $sNaechst=''; $sPw=''; $bToPayPal=false;
 $aW=array('0','0','','',''); $aFehl=array(); $bCaptcha=false; $bOK=false; $sId=''; $sSes=''; $Meld=''; $MTyp='Fehl'; $X='';
 $sClassVersion=(phpversion()>'5.3'?'':'4'); define('FRA_Session',isset($_POST['fra_Session'])?$_POST['fra_Session']:'');
   ...
   if(!FRA_NutzerSperre){
    if(($sNam=$aW[2])&&($sPw=$aW[3])){
     ...
     if(!FRA_SQL){ //Textdateien
      $aD=file(FRA_Pfad.FRA_Daten.FRA_Nutzer); $nSaetze=count($aD); $sEml=fFraEnCodeP($sNam); $sNam=fFraEnCodeP(strtolower($sNam));
      for($i=1;$i<$nSaetze;$i++){
       $a=explode(';',rtrim($aD[$i]));
       if(is_array($a)&&count($a)>3) if($a[3]==$s&&($a[2]==$sNam||$a[4]==$sEml)){ //gefunden
        $sId=$a[0]; $aW=$a; $aW[2]=fFraDeCodeP($a[2]); $aW[3]=$sPw; $aW[4]=fFraDeCodeP($a[4]);
     ...
     if($sId!=''){ //gefunden
      $Meld=FRA_TxNutzerPruefe; $MTyp='Meld'; $sForm='NutzerForm'; $sNaechst='pruefen'; $sBtn=FRA_TxWeiter;
       ...
       if($a!=$aW){ //veraendert
        if($a[2]==$aW[2]||!strpos($sNam,';'.fFraEnCodeP($aW[2]).';')){ //Benutzername unveraendert oder frei
         ...
         if($f=fopen(FRA_Pfad.FRA_Daten.FRA_Nutzer,'w')){
          ...
          $Meld=FRA_TxNutzerGeaendert; $sBtn=FRA_TxWeiter; $MTyp='Erfo'; $sPw=$aW[3];
         }else $Meld=str_replace('#',FRA_TxBenutzer,FRA_TxDateiRechte);
        }else{$Meld=FRA_TxNutzerVergeben; $aFehl[2]=true;}
       }else{ //Login fertig
        if($a[1]=='1'){ //aktiv
         $sSes=fFraSessionNr($sId); $Meld=FRA_PPMOk; $MTyp='Meld'; $sBtn=FRA_TxWeiter;


        }else{$Meld=FRA_PPMsg; $MTyp='Meld'; $bToPayPal=true; $sNaechst='pruefen'; $sBtn='Korrigieren';} //nicht aktiv
       }
      }else $Meld=FRA_TxNutzerFalsch;
     }elseif($bSQLOpen){ //bei SQL
   ...
   if($bCapOk){
    if($sNam=$aW[2]){
     if(!FRA_SQL){ //Textdateien
      ...
      for($i=1;$i<$nSaetze;$i++){
       $a=explode(';',rtrim($aD[$i]));
       if($a[2]==$sNam||$a[4]==$sEml){$sId=$a[0]; $sNam=fFraDeCodeP($a[2]); $sPass=fFraDeCodeP($a[3]); $sEml=fFraDeCodeP($a[4]); break;} //gefunden
      }
     }elseif($bSQLOpen){ //bei SQL
      if($rR=$DbO->query('SELECT * FROM '.FRA_SqlTabN.' WHERE Benutzer="'.strtolower($sNam).'" OR eMail="'.$sNam.'"')){
       if($a=$rR->fetch_row()){$sId=$a[0]; $sNam=$a[2]; $sPass=fFraDeCodeP($a[3]); $sEml=$a[4];} //gefunden
       $rR->close();
      }else $Meld=FRA_TxSqlFrage;
     }//SQL
     if(isset($sPass)){
      ...
      $s=FRA_Sender; if($p=strpos($s,'<')){$t=substr($s,0,$p); $s=substr(substr($s,0,-1),$p+1);} else $t=''; $sWww=fFraWwwP();
...
<script type=\"text/javascript\">
 ...
 //Loginmaske
 $X.='
 <p class="fraMeld" style="margin-top:20px;">'.fFraTxP(FRA_TxLoginLogin).'</p>
 ...
 <table class="fraLogi" border="0" cellpadding="0" cellspacing="0">
  <tr>
   <td class="fraLogi fra15Bs">'.fFraTxP(FRA_TxBenutzername).'<br />'.fFraTxP(FRA_TxOder).'<br />'.fFraTxP(FRA_TxMailAdresse).'</td>
   <td class="fraLogi"><input class="fraLogi" type="text" name="fra_F2" value="'.fFraTxP($aW[2]).'" maxlength="100" /></td>
  </tr>
  <tr>
   <td class="fraLogi fra15Bs">'.fFraTxP(FRA_TxPasswort).'</td>
 ...
 if($bCaptcha) $X.='<input type="hidden" name="fra_CaptchaAntwort" value="" /><input name="fra_CaptchaCode" type="hidden" value="'.$Cap->PublicKey.'" /><input name="fra_CaptchaFrage" type="hidden" value="'.fFraTxP($Cap->Question).'" /><input name="fra_CaptchaTyp" type="hidden" value="'.$Cap->Type.'" />';
 $X.="\n".' </form>';
 }else{
 $X.='
 <table class="fraLogi" border="0" cellpadding="0" cellspacing="0">
  <tr>
   <td class="fraLogi"><p class="fraFehl">'.fFraTxP(FRA_TxNutzerSperre).'</p></td>
  </tr>
 </table>';
 }
 ...
 $X.="\n".'
 <p class="fraMeld">'.fFraTxP(FRA_TxLoginVergessen).'</p>
 <form class="fraForm" action="'.FRA_Self.'" method="post">
 ...
 if(!$bOK) $X.=' <input type="submit" class="fraScha" style="background-image:url('.FRA_Http.'schalter.gif)" value="'.fFraTxP(FRA_TxSenden).'" title="'.fFraTxP(FRA_TxSenden).'" />'."\n"; else $X.='&nbsp;';
 $X.=' </form>';
 }
