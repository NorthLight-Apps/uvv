...
/*------------------------------------------------
 ...
 var $Subject='kein Betreff'; var $Text=''; var $PdfName=''; var $Attachment='';
 ...
 function PhpSend(){
  ...
  $Hd.="\r\nMessage-ID: <".$UId.'@'.$LHo.'>';
  $Hd.="\r\nContent-Type: multipart/mixed; boundary=\"bd_".$UId.'"';
  $Hd.="\r\nMIME-Version: 1.0";
  $Hd.="\r\nX-Mailer: PHP-PdfMail";
  ...
  $Tx.="\r\n";
  $Tx.="\r\n".trim(wordwrap(str_replace("\n","\r\n",str_replace("\r",'',$this->Text)),76,"\r\n"));
  $Tx.="\r\n";
  $Tx.="\r\n--bd_".$UId;
  $Tx.="\r\nContent-Type: application/".(strstr(strtolower($this->PdfName),'.')=='.pdf'?'pdf':'text').'; name="'.$this->PdfName.'"';
  $Tx.="\r\nContent-Transfer-Encoding: base64";
  $Tx.="\r\nContent-Disposition: attachment; filename=\"".$this->PdfName."\"";
  $Tx.="\r\n";
  $Tx.="\r\n".rtrim(chunk_split(base64_encode($this->Attachment),76,"\r\n"));
  $Tx.="\r\n--bd_".$UId."--\r\n";
     ...
     if($smtp->mailfrom($this->Frm)){
      ...
      if($bRcptto){
       if($smtp->data()){
        ...
        $smtp->send('Content-Type: multipart/mixed; boundary="bd_'.$UId.'"');
        $smtp->send('MIME-Version: 1.0');
        $smtp->send('X-Mailer: PHP-PdfMail via SMTP');
        $smtp->send('');
        ...
        $smtp->send('');
        $smtp->send(trim(wordwrap(str_replace("\n","\r\n",str_replace("\r",'',$this->Text)),76,"\r\n")));
        $smtp->send('');
        $smtp->send('--bd_'.$UId);
        $smtp->send('Content-Type: application/'.(strstr(strtolower($this->PdfName),'.')=='.pdf'?'pdf':'text').'; name="'.$this->PdfName.'"');
        $smtp->send('Content-Transfer-Encoding: base64');
        $smtp->send('Content-Disposition: attachment; filename="'.$this->PdfName.'"');
        $smtp->send('');
        $smtp->send(rtrim(chunk_split(base64_encode($this->Attachment),76,"\r\n")));
        $smtp->send('--bd_'.$UId.'--');
        $bRes=$smtp->complete();
    }}}}
    $smtp->quit();
